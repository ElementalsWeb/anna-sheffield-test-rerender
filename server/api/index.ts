export const listRingBands: any = [
  {
    id: "4472382750778",
...
export const listEngagementRing: any = [
  {
    id: "5519613253",
...
async function getPriceRings(client: any) {
  let productsPrice: any = {};
  let listsRingRing: any = [];
  let listsRingEngagement: any = [];

  listRingBands.forEach((rings: any) => {
    listsRingRing.push(
      Buffer.from(`gid://shopify/Product/${rings["id"]}`).toString("base64")
    );
  });

  listEngagementRing.forEach((rings: any) => {
    listsRingEngagement.push(
      Buffer.from(`gid://shopify/Product/${rings["id"]}`).toString("base64")
    );
  });

  const getPrice = async (arrayRingId: any) => {
    const productsPrice: any = {};
    await client.product.fetchMultiple(arrayRingId).then((products: any) => {
      products.map((product: any) => {
        ...
        if (product !== null) {
          ...
          let idProduct: any = Buffer.from(id, "base64")
            .toString("binary")
          ...
          variants.map((item: any) => {
            ...
            let priceV2: any = item.priceV2.amount;
          ...
          productsPrice[idProduct] = prices;
        }
      });
